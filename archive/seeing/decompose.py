"""Decompose VLM correctness into vision-driven vs prior-driven channels.
...
i.e. of all *correct rollouts* the model produced, what fraction
came from examples where seeing the image actually moved belief
toward the gold answer.
"""
...
# g-density bin edges for the accountability summary.
...
class DecompResult:
    ...
    pass_rate_g_pos: float                  # mean pass_rate | g > 0
    pass_rate_g_nonpos: float               # mean pass_rate | g <= 0
...
def decompose(
    ...
    *,
    ...
    that already contains teacher-forced sum log-probs and per-rollout
    correctness lists (PEAR-6 schema).
    """
    ...
    if noise_floor_nat is not None:
        ...
        if n_below:
            notes.append(
                f"dropped {n_below} rows with m_img_sum <= {noise_floor_nat:g}"
    ...
    # Correct rollouts mass on each row = pass_rate × G_rollouts. Since
    # the row-count of rollouts is constant, we work with pass_rate
    # directly: vision_driven_frac is invariant under that scaling.
    g_pos = g > 0
    g_neg = g < 0
    g_nonpos = ~g_pos
...
def format_result(r: DecompResult) -> str:
    ...
    lines.append("")
    lines.append(f"   ρ(g, pass_rate) = {r.rho_g_passrate:+.4f}    "
...
def _synthetic_smoke() -> Sequence[DecompResult]:
    """Hand-built tiny dataset for verification.

    Five examples. Expectations:
      - rows 1,2,3 have g > 0 (image helped)  pass_rates [0.50, 0.75, 1.00]
      - rows 4,5   have g <= 0                pass_rates [0.25, 0.50]
      - sum_pr_pos = 2.25 ; total = 3.00 ; vision_driven_frac = 0.75
      - rho(g, pr) should be strictly positive
    """
    df = pd.DataFrame({
        "m_img_sum":   [-3.0, -2.0, -1.0, -5.0, -4.0],
        "m_blank_sum": [-4.0, -4.0, -4.0, -4.0, -4.0],
        "pass_rate_g16": [0.50, 0.75, 1.00, 0.25, 0.50],
        "correct_g16_eval": [[1]*8 + [0]*8, [1]*12 + [0]*4,
                             [1]*16, [1]*4 + [0]*12, [1]*8 + [0]*8],
    })
